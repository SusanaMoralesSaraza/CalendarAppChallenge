...
@dataclass
class Reminder:
    EMAIL: ClassVar[str] = "email" # variables de clase
...
@dataclass
class Event:
    title: str #parametros obligatorios
    description: str
    date_: date
    start_at: time
    end_at: time
    reminders: list[Reminder] = field(init=False, default_factory=list) #cuando no se inicializa con parámetros
    id: str = field(default=generate_unique_id) #defaul retorna la función por eso no se pone el paréntesis
...
class Day:
    def __init__(self, date_: date):
        ...
        self._init_slots()  #invoco un metodo
...
class Calendar:
    ...
    def add_event(self, title: str, description: str, date_: date, star_at: time, end_at: time):
        ...
        self.days[day_].add_event(event_id, start_at, end_at)
